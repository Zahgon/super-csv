...
public class CsvDozerBeanReader extends AbstractCsvReader implements ICsvDozerBeanReader {
     ...
     */
    public CsvDozerBeanReader(final Reader reader, final CsvPreference preferences, final DozerBeanMapper dozerBeanMapper) {
        super(reader, preferences);
     ...
     */
    public CsvDozerBeanReader(final ITokenizer tokenizer, final CsvPreference preferences, final DozerBeanMapper dozerBeanMapper) {
        super(tokenizer, preferences);
        if (dozerBeanMapper == null) {
            throw new NullPointerException("dozerBeanMapper should not be null");
        }
    ...
    private static class MappingBuilder extends BeanMappingBuilder {
        ...
        public MappingBuilder(final Class<?> clazz, final String[] fieldMapping, final Class<?>[] hintTypes) {
            if (clazz == null) {
                ...
                throw new IllegalArgumentException(String.format("hintTypes length(%d) should match fieldMapping length(%d)", hintTypes.length, fieldMapping.length));
            }
            this.clazz = clazz;
            this.fieldMapping = fieldMapping;
