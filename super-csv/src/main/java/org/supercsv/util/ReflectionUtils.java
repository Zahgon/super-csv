...
public final class ReflectionUtils {
     ...
     */
    private static Method findGetterWithCompatibleReturnType(final String getterName, final Class<?> clazz, final boolean enforceBooleanReturnType) {
        for (final Method method : clazz.getMethods()) {
            if (!getterName.equalsIgnoreCase(method.getName()) || method.getParameterTypes().length != 0 || method.getReturnType().equals(void.class)) {
                // getter must have correct name, 0 parameters and a return type
                continue;
            }
            if (!enforceBooleanReturnType || boolean.class.equals(method.getReturnType()) || Boolean.class.equals(method.getReturnType())) {
                return method;
            }
        }
        return null;
     ...
     */
    private static Method findSetterWithCompatibleParamType(final Class<?> clazz, final String setterName, final Class<?> argumentType) {
        ...
        while (actualType != null && compatibleSetter == null) {
            ...
            for (Method method : methods) {
                if (!setterName.equalsIgnoreCase(method.getName()) || method.getParameterTypes().length != 1) {
                    // setter must have correct name and only 1 parameter
                    continue;
                }
                ...
                if (parameterType.equals(argumentType)) {
                    ...
                    // potential match, but keep looking for exact match
